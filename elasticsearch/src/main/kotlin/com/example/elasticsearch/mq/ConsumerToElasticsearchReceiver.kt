...
import com.example.elasticsearch.service.ConsumerDocService
import com.example.elasticsearch.service.ConsumerService
...
class ConsumerToElasticsearchReceiver {
    ...
    @Autowired
    lateinit var consumerDocService: ConsumerDocService
    ...
    fun process(channel: Channel, message: Message) {
        ...
        mqMessage.authorization?.let {
            if (mqMessage.modelType == "consumer" && mqMessage.modelId > 0) {
                val response = consumerService.consumerShow(mqMessage.modelId, it)
                ...
                if (consumer != null && producer != null) {
                    val consumerDoc = ConsumerDoc(consumer = consumer, producer = producer)
                    consumerDocService.save(consumerDoc)
                    log.info("save ${consumerDoc.toString()}")
                }
            }
        }
    }
